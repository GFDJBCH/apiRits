...
class Catalogos extends Connection
{
    ...
    private $table = "_UnidadesMedida";
    private $tableCatalogo = "_Catalogos";
    private $Id = 'null';
    ...
    public function listaUmc($page = 1)
    {
        $start = 0;
        $qty = 50;
        if ($page > 1):
        ...
        $query = "SELECT * FROM " . $this->table . " WHERE Tipo = 'UMC' limit $start,$qty";
        $data = parent::getData($query);
        return ($data);
    }
    public function listaUmt($page = 1)
    {
        $start = 0;
        $qty = 50;
        if ($page > 1):
        ...
        $query = "SELECT * FROM " . $this->table . " WHERE Tipo = 'UMCOVE' limit $start,$qty";
        $data = parent::getData($query);
        return ($data);
    }
    ...
    public function listaClavePedimento($page = 1)
    {
        $start = 0;
        $qty = 50;
        if ($page > 1):
        ...
        $query = "select Id as id, ValorTexto as text from _Catalogos where Tipo = 'ClavesPedimentos';";
        $data = parent::getData($query);
        return ($data);
    }
    public function listaTipoOperacion($page = 1)
    {
        $start = 0;
        $qty = 50;
        if ($page > 1):
        ...
        $query = "select Id as id, ValorTexto as text from _Catalogos where Tipo = 'TIPOOPERACION' and Codigo in (1,2);";
        $data = parent::getData($query);
        return ($data);
    }
    public function listaTipoStatus()
    {
        $query = "SELECT * FROM _Catalogos WHERE Tipo = 'CDCESTATUS' ORDER BY Orden;";
        $data = parent::getData($query);
        return ($data);
    }
    public function listaSucursales()
    {
        $query = "select sucursal_id as id, sucursal_codigo as text, sucursal_descripcion as descripcion from vw_wms_revision where sucursal_id is not null group by sucursal_id;";
        $data = parent::getData($query);
        return ($data);
    }
    public function listaZona($page = 1)
    {
        $start = 0;
        $qty = 50;
        if ($page > 1):
        ...
        $query = "SELECT Id as id, concat_ws(' - ', Codigo, ValorTexto) as text FROM " . $this->tableCatalogo . " WHERE Tipo = 'Zona' limit $start,$qty";
        $data = parent::getData($query);
        return ($data);
    }
